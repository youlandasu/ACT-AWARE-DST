...
@Predictor.register('act_aware_dst')
class ADSTPredictor(Predictor):
    def __init__(self, model: Model, dataset_reader: DatasetReader, language: str = 'en_core_web_sm') -> None:
        super().__init__(model, dataset_reader)
        domain_slot_list_path = "./ontology/domain_slot_list_sp.txt"  # if no span, use "./ontology/domain_slot_list.txt"
        ...
        self._ds_list, self._ds_text2id, value_path_list, self._ds_type, self._ds_use_value_list = self.read_domain_slot_list(
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        ...
        for turn_i, turn in enumerate(dialog):
            ...
            span_labels.append(cur_span_idx)
            acts.extend([Token(text="<S>", lemma_="<S>")] + token_act + [Token(text="</S>", lemma_="</S>")])
            act_list.append(acts.copy())

        instance = self._dataset_reader.text_to_instance(dialog_idx, dialog_context, exact_match_feas, tags, utt_lens, labels, spans, span_labels,acts)
        return instance
