...
from allennlp.data.fields import Field, TextField, \
    MetadataField, LabelField, ListField, ArrayField

from act_aware_dst import dstqa_util

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

@DatasetReader.register("act_aware_dst")
class ADSTReader(DatasetReader):
    def __init__(self,
        ...
        self._ds_list, self._ds_text2id, value_path_list, self._ds_type, self._ds_use_value_list, self._ds_masked = self.read_domain_slot_list(
            domain_slot_list_path)

        # empty span list if nosp
        ...
        for domain_slot in self._ds_list:
            ...
            self._value_id2text[domain_slot], self._value_text2id[domain_slot] = self.read_value_list(
                domain_slot_list_path, domain_slot, value_path_list)
    ...
    def _read(self, file_path: str):
        ...
        for dialog in dataset_json:
            ...
            for turn_i, turn in enumerate(dialog):
                ...
                sys_act = turn['system_acts']
                token_act = []
                if sys_act is not None:
                    for item in sys_act:
                        token_act.append(Token(text=item, lemma_=item))
                ...
                cur_labels = []
                cur_spans = []
                cur_span_idx = []  # 0: none; 1: dont care; 2: span
                    ...
                    for slot, value in val.items():
                        ...
                        if ds in self._ds_type and self._ds_type[ds] == "span":
                            ...
                            if value == "dont care":
                                ...
                                sl = 2
                            cur_span_idx.append((domain, slot, sl))
                labels.append(cur_labels)
                spans.append(cur_spans)
                span_idx.append(cur_span_idx)
                acts.extend([Token(text="<S>", lemma_="<S>")] + token_act + [Token(text="</S>", lemma_="</S>")])
                act_list.append(acts.copy())
                
            instance = self.text_to_instance(dialog_idx, dialog_context, exact_match_feas, tags, utt_lens, labels,
                                             spans, span_idx,acts)
            yield instance
    ...
    def text_to_instance(self, dialog_idx,
                         ...
                         utt_lens, labels, spans, span_labels,acts):
        ...
        fields['system_acts'] = TextField(acts, token_indexers) 
        
        # calculate labels
        if labels != None:
            expanded_value_labels = []
            for turn_label in labels:
                turn_value_label = [-1 if self._ds_type[ds] == "span" else 0 for ds in
                                    self._ds_list]  # 0 is default which is 'none' is in vocab
